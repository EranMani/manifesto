# Commit 46 - `policy-grounded-prompt` - Nova
...
Construct an injection-resistant prompt from labelled policy evidence.

---

## Semantic Fit Review

- **Atomic outcome:** One prompt builder consumes a frozen context contract.
- **Failure boundary:** Streaming remains C47-C49.
...
| File | Type | Purpose |
|---|---|---|
| `backend/app/services/rag_policy.py` | edit | Construct an injection-resistant prompt from labelled policy evidence. |
| `backend/tests/services/test_rag_policy.py` | edit | Prove policy-grounded-prompt |
...
```powershell
docker compose run --rm backend uv run pytest tests/services/test_rag_policy.py -k grounded_prompt -q
```
...
- Later policy RAG behavior starts C47.
