# Commit 41 - `policy-rank-fusion` - Nova
...
Fuse vector and lexical candidate ranks deterministically.

---

## Semantic Fit Review

- **Atomic outcome:** One pure ranking transform combines two frozen candidate lists.
- **Failure boundary:** Diversification remains C42.
...
| File | Type | Purpose |
|---|---|---|
| `backend/app/services/rag_policy.py` | edit | Fuse vector and lexical candidate ranks deterministically. |
| `backend/tests/services/test_rag_policy.py` | edit | Prove policy-rank-fusion |
...
```powershell
docker compose run --rm backend uv run pytest tests/services/test_rag_policy.py -k rank_fusion -q
```
...
- Reciprocal-rank scores are stable.
- Ties use stable IDs.
...
- Later policy RAG behavior starts C42.
