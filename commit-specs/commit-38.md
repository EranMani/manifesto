# Commit 38 - `policy-query-embedding` - Nova
...
- **Atomic outcome:** Query normalization and embedding form one provider-neutral input step.
- **Failure boundary:** Database retrieval remains C39-C40.
...
| File | Type | Purpose |
|---|---|---|
| `backend/app/services/rag_policy.py` | edit | Normalize a policy query and embed it with the active profile. |
| `backend/tests/services/test_rag_policy.py` | new | Prove policy-query-embedding |
...
- Normalized text is embedded once.
- Blank input is rejected without provider work.
...
- Later policy RAG behavior starts C39.
