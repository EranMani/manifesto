...
| DevOps Engineer | Adam | sonnet | Infrastructure plus workflow automation under `hooks/` (except `hooks/agent-config.json`, `hooks/tool_cap_end.py`, `hooks/tests/test_tool_cap.py`, `hooks/pre_commit_check.py`, and `hooks/tests/test_pre_commit_check.py` — a narrow Claude/orchestrator exception for the agent identity registry, token telemetry, and the commit-gate hook that enforces this protocol) |
