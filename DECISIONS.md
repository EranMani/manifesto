...
- **Follow-up gap (fixed same session):** the C47 chore(state) sweep then
  failed `pre_commit_check.py`'s domain-boundary check on
  `.context/direct/C47.md` — `hooks/agent-config.json`'s allowed prefixes for
  `claude@anthropic.com` included `.context/finalize/` but not
  `.context/direct/`, so the brief file generated by
  `prepare_claude_direct.py` could never be committed as CLAUDE.md step 12.3
  requires, and `git status` could never go clean after a Claude-direct
  commit. Fixed by adding `.context/direct/` alongside `.context/finalize/`
  in Claude's allowed prefixes (Eran-approved).
